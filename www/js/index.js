...
$(document).off('click', '#submit').on('click', '#submit', function() { 
    ...
    if($('#username').val().length > 0 && $('#password').val().length > 0){
        ...
        $.ajax({url: 'http://insurance.fourthforce.in/slim-insu-quest/index.php',
    ...
    if(pas != ''){
     ...
     /*   sessionStorage.setItem("supervisor_new_array",JSON.stringify(result[1]));
      ...
      /*  sessionStorage.setItem("supervisor_activity_log_array",JSON.stringify(result[13]));*/
        ...
        sessionStorage.setItem("driver_details_d_array",JSON.stringify(result[48]));
        sessionStorage.setItem("driver_details_o_array",JSON.stringify(result[49]));
        sessionStorage.setItem("fir_details_array",JSON.stringify(result[50]));


        sessionStorage.setItem("accident_details_array",JSON.stringify(result[51]));
...
$(document).off('click', '#chngpass').on('click', '#chngpass', function() { 
  //alert('chngpass');
        ...
        console.log($('#check_userpass').serialize());
                    $.ajax({url: 'http://insurance.fourthforce.in/slim-insu-quest/chngpass.php',
